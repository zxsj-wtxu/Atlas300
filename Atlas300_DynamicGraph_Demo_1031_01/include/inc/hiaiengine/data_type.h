...
#include "hiaiengine/ai_nn_tensor_list.h"
...
namespace hiai {
    // HIAIEngine 集成已支持普通序列化和反序列化的数据类型
    ...
    // name: "NNTensorList"                         type:NNTensorList
    ...
    struct ImageData {
        IMAGEFORMAT format = YUV420SP; // 图像格式
    ...
    template<class Archive>
    void serialize(Archive& ar, NNTensorDescription& data)
    {
        ar(data.dataSize, data.tensorNumber, data.channels, data.height, data.width,
           data.dataType, data.name);
    }
    
    // 用户自定义的数据类型必须先注册到HIAIEngine，才能作为Engine之间的通信接口正常使用
    template<class Archive>
    void serialize(Archive& ar, NNTensorList& data)
    {
        if (data.GetBufferSize() > 0 && data.GetBuffData() == nullptr) {
            data.SetBuffData(new(std::nothrow) uint8_t[data.GetBufferSize()]);
        }
        uint32_t tmpNNTensorNumber = data.GetNNTensorNumber();
        // 此处临时复制一份nnTensorVector
        std::vector<NNTensorDescription> nnTensorVec = data.GetNNTensorVector();
        ar(tmpNNTensorNumber);
        ar(nnTensorVec);
        if (tmpNNTensorNumber != data.GetNNTensorNumber()) {
            data.SetNNTensorNumber(tmpNNTensorNumber);
            data.SetNNTensorVector(nnTensorVec);
        }
    }

...
extern void* create_collect_client(const char* target, const char* engine_name);
 ...
 * \return 0 on success
 */
extern int collect_host_sync_dev_list(void* handle, const char* job_ctx, const collect_dev_info_s* dev_list, int dev_num);
...
extern int collect_write(void* handle, const char* job_ctx, struct data_chunk* data);
...
extern void release_collect_client(void* handle);

// TODO (#1#): extern int register_sampling_switch_config(const unsigned char* config_str);// on-off the sampling config. First way to set sampling config
// TODO (#2#): extern int register_sampling_switch_func(struct switch_operation* opt);// on-off the sampling config func. Second way to set sampling config
 ...
 */
extern char* collect_dev_update_job_ctx(const char* job_ctx, const collect_dev_info_s * info);
#ifdef __cplusplus
}
#endif // __cplusplus

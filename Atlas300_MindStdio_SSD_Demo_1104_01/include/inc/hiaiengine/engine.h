...
namespace hiai {
    static constexpr uint32_t TIME_OUT_VALUE = 500;     // 500 ms
    static constexpr int32_t ENGINE_MAX_QUEUE_SIZE = 200;
    static constexpr int32_t QUEUE_START_INDEX = 0;
    static constexpr int32_t IN_QUEUE = 0;
    static constexpr int32_t OUT_QUEUE = 1;
    static constexpr uint32_t REMOVE_MODEL_FILE_FLAG = 0;

    // engine接收数据超时处理机制信息结构体
    struct EngineRecvDataTimeoutInfo {
        uint32_t waitMaxTime;  // 超时时间,单位ms
        bool isRepeatTimeout;  // 是否反复超时
        bool isSingleTimeout;  // 是否单次超时
    };

    // 预先声明Engine需要使用的类和相关数据结构
    template <typename T>
    class QueueManager;
    class Worker;
    class EngineConfig;
    ...
    class Engine {
      ...
      public:
        ...
        */
        HIAI_LIB_VISIBILITY const uint32_t GetQueueMaxSize() const;
        ...
        */
        HIAI_LIB_VISIBILITY const uint32_t GetQueueCurrentSize(
            const uint32_t portID) const;
        /** @ingroup hiaiengine_common
        *  @brief    GetOamConfig       : 获取OamConfig智能指针
        *  @return   OAMConfigDef*      : OAMConfigDef指针
        */
        HIAI_LIB_VISIBILITY OAMConfigDef* GetOamConfig();
      protected:
        /**
        * @ingroup hiaiengine
        ...
        */
        ...
        /**
        * @ingroup hiaiengine
        * @brief 检测是否需要通过对端engine转发数据
        * @param [in] targetEnginePortList : 目标engine列表
        */
        HIAI_LIB_INTERNAL bool CheckIsNeedTransDataInTargetEngines(
            const std::list<EnginePortID>& targetEnginePortList);
        ...
        */
        ...
        /** @ingroup hiaiengine_common
        *  @brief    SetDumpCommand   : 根据oamConfig配置设置Dump
        *  @param    [in]oamConfigDef : oamConfig
        *  @return   HIAI_StatusT
        */
        HIAI_LIB_INTERNAL HIAI_StatusT SetDumpCommand(const OAMConfigDef& oamConfigDef);
        /** @ingroup hiaiengine_common
        *  @brief    UpdateDump       : 更新Dump
        *  @return   HIAI_StatusT
        */
        HIAI_LIB_INTERNAL HIAI_StatusT UpdateDump();
      private:
        /**
        * @ingroup hiaiengine
        * @brief 初始化队列管理类
        */
        ...
        /**
        * @ingroup hiaiengine
        * @brief 更新engine缓存队列大小上限
        * @param [in] updateDirection 更新方式，true限制上限为当前队列大小，false解除限制，一次按5个缓存大小逐渐递增恢复
        */
        HIAI_LIB_INTERNAL void UpdateMaxQueueSize(bool updateDirection);
      ...
      private:
        ...
        uint32_t graphID_;
        uint32_t inPortNum_;
        uint32_t outPortNum_;
        std::mutex  mutexHandle_;
        std::shared_ptr<EngineConfig> configPtr_;
        EngineRecvDataTimeoutInfo recvDataTimeoutInfo_;
        // Out port id和DataRecvInterface的映射表
        std::recursive_mutex dataRecvMapMutex_;
        std::map<uint32_t, std::shared_ptr<DataRecvInterface> > dataRecvMap_;

        std::shared_ptr<QueueManager<EngineQueueItem>> queueManager_;
        std::shared_ptr<Worker> threadWorker_;
        thread_local static uint64_t reqID_;
        uint64_t jobID_;
    };
